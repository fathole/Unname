...
namespace TacticsToolkit
{
    public class AIController : Entity
    {
        ...
        public List<Entity> allCharacters;

        //List of players in the game
        public List<Entity> enemyCharacters;
        private List<Entity> allyCharacters;
        ...
        public void Start()
        {
            allCharacters = new List<Entity>();
            allCharacters.AddRange(GameObject.FindGameObjectsWithTag("Player").Select(x => x.GetComponent<Entity>()).ToList());
            allCharacters.AddRange(GameObject.FindGameObjectsWithTag("Enemy").Select(x => x.GetComponent<Entity>()).ToList());

            //getTeams
            enemyCharacters = allCharacters.Where(x => x.teamID != teamID).ToList();
            allyCharacters = allCharacters.Where(x => x.teamID == teamID).ToList();
        ...
        private Entity FindClosestCharacter(OverlayTile position)
        {
            ...
            foreach (var player in enemyCharacters)
            {
                if (player.isAlive)
                {
